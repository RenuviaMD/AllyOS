  ...
  const system = [
    ...
    },
    {
      ...
      text:
        ...
        "Draft protocols/ingredients staged for future promotion (currently Niagen/NR — NAD+ precursor, distinct from IV NAD+, no peptide synergy). TWO SEPARATE SIGN-OFFS — keep them distinct: (1) EDITORIAL/curator sign-off is DONE — the AllyOS curator (Falcon, MD) has already PUBLISHED this entry as reference, with verified citations; this is editorial, not a prescription. (2) CLINICAL use is what remains: the entry sits OUTSIDE the locked primary library (in_primary_library:false, DRAFT badge, does NOT auto-fire), and to actually administer it the clinic's MEDICAL DIRECTOR (MD/DO) must review and sign a standing order authorizing the clinic's RNs to administer it — AllyOS never prescribes. So do NOT say 'requires the Medical Director's editorial sign-off' (that step is done); say 'published as reference; clinical use requires the clinic's Medical Director to sign a standing order, and injectable NR is commonly 503B-compounded — VERIFY current FDA 503B status.' Use only the verified citations carried in the entry.\n\nDRAFTS:\n" + DRAFTS +
