...
exports.handler = async (event) => {
  ...
  // ALL modules whenever the query is ambiguous, cross-line, or matches nothing, so
  // no grounding is ever silently dropped. Cuts the cached payload ~2-3x on single-
  // line questions. Doses come from the locked fields either way — this changes cost,
  // not clinical data. The preambles/guardrails always stay; only the heavy JSON body
  // of an off-line module is swapped for a short stub.
  const qtext = messages.map(function (m) { return typeof m.content === "string" ? m.content : ""; }).join(" ").toLowerCase();
  ...
  try {
    const resp = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        // Sonnet 5 for the live grounded chat — matched Opus on every guardrail in
        // the A/B (scripts/model-ab.py) at ~1/3 the cost. Opus stays on the monthly
        // ops-audit (judgment-heavy, low volume). Doses come from locked fields.
